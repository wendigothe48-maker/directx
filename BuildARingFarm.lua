...
pcall(function()
    ...
    if playerGui then
        local sf = playerGui:FindFirstChild("MainUI") 
            ...
            and playerGui.MainUI.Menus.IndexFrame.Main:FindFirstChild("ScrollingFrame")
...
MainSection:Toggle({
    ...
    Callback = function(state)
        ...
        if getgenv().autoSeedAndBuyEnabled then
            task.spawn(function()
                while getgenv().autoSeedAndBuyEnabled do
                    local success, err = pcall(function()
                        local remotes = game:GetService("ReplicatedStorage"):WaitForChild("Remotes")
                        ...
                        for _, stand in ipairs(sr:GetChildren()) do
                            ...
                            if id then
                                ...
                                if cube then standsMap[id] = cube.Position end
                            end
                        end
                        ...
                        for _, obj in ipairs(workspace:GetChildren()) do
                            if obj:IsA("Model") and obj.Name ~= "Map" and not game:GetService("Players"):GetPlayerFromCharacter(obj) then
                                local sg = obj:FindFirstChild("SeedGui", true)
                                local pivot = obj:GetPivot()
                                if sg and pivot then
                        ...
                        for _, info in ipairs(mySeeds) do
                            ...
                            local rInfo = info.sg:FindFirstChild("InfoFrame", true)
                            if rInfo then
                            ...
                            if mustBuy then
                                ...
                                if getPlayerCash() >= cost then
                                    buySeed:FireServer(info.standId)
                                    task.wait(1)
                                    actedOnSeed = true
                                    break
                                else
